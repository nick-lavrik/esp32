...
public:
     ...
     */
    static String format(uint64_t bytes,
                          uint8_t precision = 2,
     ...
     */
    static size_t formatTo(char* outBuffer,
        ...
        int written = std::snprintf(outBuffer, outBufferSize, "%.*f %s",
                                     precision, value, units[unitIndex]);
        return clampWritten(written, outBufferSize);
    }
