...
4. `monitorResetOnConnect=false` + `monitorDtrOnConnect=false` — **не є захистом на C6**, див. 3.5.
...
### 3.5. На ESP32-C6 відкриття порту ресетить чип — незалежно від налаштувань

Перевірено емпірично 25.08.2026 на живій платі. Порт відкривався з `dtr=False`, `rts=False`, виставленими **до** `open()`:
...
Друга сесія знову починається з ROM boot і uptime з нуля — тобто **чип ресетнувся від самого факту відкриття CDC-порту**. Те саме видно в `pio device monitor`, який явно друкує `--- forcing DTR inactive` / `--- forcing RTS inactive` і все одно отримує `rst:0x15 (USB_UART_HPSYS)`.

Це властивість вбудованого USB-Serial/JTAG контролера ESP32-C6, а не плагіна і не PlatformIO. Наслідки:

- `monitorResetOnConnect=false` і `monitorDtrOnConnect=false` **не гарантують** відсутність ресету на цій платі. Вони корисні (прибирають *додаткові* ресети та зависання монітора), але не є бар'єром;
- будь-яке відкриття монітора = перезапуск прошивки. Стан у RAM, uptime, незаписані дані втрачаються;
- щоб зловити логи саме з моменту старту, монітор треба відкривати **до** цікавої події — після відкриття подія все одно почнеться з нуля;
- єдиний спосіб не ресетнути плату — не відкривати порт. Звідси й побудова захисту в 3.4.
...
Для простого читання логів PTY не потрібен — досить pyserial без торкання DTR/RTS (але ресет чипа все одно буде, розділ 3.5).
...
Врахуй 3.5: на ESP32-C6 будь-яке відкриття порту ресетить чип,
...
- **розділ 3.5** — на ESP32-C6 відкриття порту ресетить чип незалежно від DTR/RTS і налаштувань плагіна. Доведено двома послідовними сесіями читання: обидві починаються з ROM boot, uptime з нуля. Це змінює модель захисту в 3.4: `monitorResetOnConnect=false` перестає бути бар'єром, бар'єр — «порт ніхто не відкриває сам»;
