...
* **Немає `type`, `ifname`, VPN, bridge/bond/vlan** — інтерфейс рівно один,
  `wlan0`.
* **У прошивці є свій перелік мереж.** На кожному старті відсутні SSID із
  таблиці `kWifiNetworks` (`src/WifiNetworks.hpp`) додаються до списку, щоб
  плата після чистої прошивки не лишилась без зв'язку. Вже збережені профілі
  при цьому не чіпаються — див. розділ нижче.
...
* **на старті** файли читаються, лише якщо список профілів у NVS **порожній**
  ...
  `NVS → /network/*.nmconnection → kWifiNetworks з прошивки`. Останній крок,
  на відміну від двох перших, робиться завжди — але лише для SSID, яких у
  списку ще немає;
...
### Прошитий перелік мереж (`src/WifiNetworks.hpp`)

Третє, найглибше джерело — таблиця просто в коді. Головний її сенс: плата з
чистою NVS і порожнім LittleFS має до чогось підключитись, інакше єдиний шлях
до неї — AP-fallback і портал.
...
Засівання робить `NetworkSupervisor::seedConnections(kWifiNetworks)` — механіка
в бібліотеці, дані в прошивці:
...
* **Наявний профіль не перезаписується — навіть якщо пароль у таблиці інший.**
  Після першого старту джерело істини — NVS, інакше кожен ребут відкочував би
  зроблене через `net`. Щоб розбіжність не була мовчазною, у лог іде
  попередження:

  ```
  [net] profile 'Asus ' kept from storage; built-in password differs
  ```

  Тобто «змінив пароль у `secrets.ini`, перепрошив, а плата не конектиться»
  лікується не перепрошивкою, а `net connection modify <id> wifi-security.psk <новий>`.
* **Видалену прошиту мережу ребут повертає.** `net connection delete` прибирає
  профіль із NVS, але наступний старт засіє його з таблиці знову. Щоб вимкнути
  таку мережу назовсім — `net connection modify <id> connection.autoconnect no`:
  профіль лишається в NVS, засів його не чіпає, а FSM його пропускає.
