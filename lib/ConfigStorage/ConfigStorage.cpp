...
String ConfigStorage::namespaceDir() const {
    return "/.nvs/" + namespaceName_;
}
...
std::vector<ConfigStorage::Entry> ConfigStorage::listEntries() {
    ...
    while (dir.next()) {
        ...
        if (ext == "str") entry.type = NVS_TYPE_STR;
        else if (ext == "i32") entry.type = NVS_TYPE_I32;
        else if (ext == "u32") entry.type = NVS_TYPE_U32;
        else entry.type = NVS_TYPE_BLOB; // f32/bool/bin — на NVS теж зберігались би як blob
