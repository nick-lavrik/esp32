...
public:
    ...
    explicit AnalogSensor(uint8_t adcPin, uint16_t adcMinValue = 0, uint16_t adcMaxValue = 4095, uint16_t mapMin = 0, uint16_t mapMax = 4095, uint16_t mapThreshold = 1)
        : _adcPin(adcPin), _adcMinValue(adcMinValue), _adcMaxValue(adcMaxValue), _mapMin(mapMin), _mapMax(mapMax), _mapThreshold(mapThreshold) {
    ...
    void update() {
        ...
        uint16_t actual = static_cast<uint16_t>(constrain(value, std::min(_mapMin, _mapMax), std::max(_mapMin, _mapMax)));
    ...
    void removeListener(AnalogSensorListenerId id) {
        _listeners.erase(
            std::remove_if(_listeners.begin(), _listeners.end(), [id](const Entry& entry) { return entry.id == id; }),
            _listeners.end()
        );
    }
