...
AnalogSensorMqttReporter::AnalogSensorMqttReporter(AnalogSensor& sensor,
                                            MqttClient& mqttClient,
                                            const char* topic,
                                            uint32_t intervalMs,
                                            uint8_t deltaThresholdPercent)
    : _sensor(sensor), _mqttClient(mqttClient), _topic(topic),
      _intervalMs(intervalMs), _deltaThresholdPercent(deltaThresholdPercent) {
}
