...
extern "C" uint32_t sntp_update_delay_MS_rfc_not_less_than_15000() { return 60000; }
#else
#include "esp_sntp.h"
#endif
...
void NtpService::begin(long gmtOffsetSec, int daylightOffsetSec, const char* ntpServer1,
                       const char* ntpServer2, const char* ntpServer3, uint32_t syncIntervalMs) {
...
bool NtpService::removeCallback(NtpCallbackHandle handle) {
  auto it = std::remove_if(_callbacks.begin(), _callbacks.end(),
...
const char* NtpService::_formatTime(const char* format, char* buffer, size_t max, time_t sec,
