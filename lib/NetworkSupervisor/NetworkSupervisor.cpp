...
void NetworkSupervisor::begin() {
  ...
  // Профілі та їхні паролі й так живуть у нашому сховищі (ConfigStorage,
  // ключ nm_conn) - штатний NVS-блоб WiFi-ядра (nvs.net80211/ap.passwd) тут
  // зайвий: FSM сам вирішує, коли й до чого конектитись
  // (WiFi.setAutoReconnect(false) нижче), тож дублювати профіль у флеш
  // ядра - зайвий запис на кожен WiFi.begin() і зайва копія пароля в NVS
  // (яку видно в розділі NVS порталу як привілейовану секцію).
