...
std::string EspPartitionInspector::detectState(const esp_partition_t* partition) {
  ...
  if (partition->type == ESP_PARTITION_TYPE_APP) {
    return (header[0] == kEspAppImageMagic) ? PartitionState::ValidAppImage
...
std::vector<EspPartitionInfo> EspPartitionInspector::collectAllEsp8266() {
  ...
  flashTotal.subtypeName =
      (flashChipSize == flashChipRealSize) ? "size-matches-real" : "size-mismatch";
  flashTotal.offset = 0;
...
void EspPartitionInspector::printOne(const EspPartitionInfo& info, Print& out) {
  out.printf("%-14s %-7s %-17s 0x%08X 0x%08X %-4s %-8s %s\n", info.label.c_str(),
             info.typeName.c_str(), info.subtypeName.c_str(),
             static_cast<unsigned int>(info.offset), static_cast<unsigned int>(info.size),
             info.encrypted ? "yes" : "no", info.readOnly ? "yes" : "no", info.state.c_str());
