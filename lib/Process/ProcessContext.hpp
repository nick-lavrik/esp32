...
public:
  ...
  bool report(const TProgress& value) { return _state->progressQueue.tryPush(value); }
  ...
  bool isCancelled() const { return _state->cancelRequested.load(std::memory_order_relaxed); }
  ...
  void fail() { _state->status.store(ProcessStatus::Failed, std::memory_order_release); }
