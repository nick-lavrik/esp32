...
public:
  ...
  bool isDone() const {
    auto s = status();
    return s == ProcessStatus::Completed || s == ProcessStatus::Cancelled ||
  ...
  bool tryGetProgress(TProgress& out) { return valid() && _state->progressQueue.tryPop(out); }

  /// Прочитати фінальний результат. Має сенс лише після isDone() зі статусом Completed.
  bool tryGetResult(TProgress& out) const { return valid() && _state->getFinalResult(out); }
